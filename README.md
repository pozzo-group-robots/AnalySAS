## analysas
[![Build Status](https://travis-ci.org/uwescience/analysas.svg?branch=master)](https://travis-ci.org/uwescience/analysas)
...
`analysas/analysas.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `analysas.py` in directory called
`analysas`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import analysas as sb` in an
interactive Python session, the classes and functions defined inside of the
`analysas.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the analysas module, the main classes defined at the bottom of
...
Either way, you can create a `analysas/data` folder in which you can
...
(typically, something like our `analysas/tests/test_analysas.py`). Within each
...
have one such test in `analysas/tests/test_analysas.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/analysas`),
and type:
...
will see a message such as:
    ...
    analysas/tests/test_analysas.py .F...
    ...
    analysas/tests/test_analysas.py:49: AssertionError
...
To document `analysas` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/analysas/blob/master/doc/_static/RTD-advanced-conf.png)

 http://analysas.readthedocs.org/en/latest/
...
A `analysas/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/analysas) for the software. This
...
`analysas` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `analysas`, we use the
...
[here](https://github.com/uwescience/analysas/blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "analysas" repository.
...
`analysas/analysas.py` to be called `smallish/smallish.py`

	git mv analysas smallish
	git mv smallish/analysas.py smallish/smallish.py
	git mv smallish/tests/test_analysas.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `analysas` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `analysas` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `analysas` in them. Type
the following to see where all these files are:

	git grep analysas

You can replace `analysas` for `smallish` quickly with:

	git grep -l 'analysas' | xargs sed -i 's/analysas/smallish/g'
...
Edit all the mentions of `analysas` in the `analysas/__init__.py` file, and in
the `analysas/version.py` file as well.
